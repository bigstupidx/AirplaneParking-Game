...
private function DeformNode(T : Transform, originalLocalPos : Vector3, originalLocalRot : Quaternion, contactPoint : Vector3, contactVector : Vector3)
	{
	var dist = (contactPoint-T.position).magnitude;
	
	// Deformar posición
	
	if (dist < deformRadius)
		{
		var posDeformForce = (deformRadius - dist) / deformRadius;
		T.position += contactVector * posDeformForce + Random.onUnitSphere * deformNoise;
	...
	if (dist < deformNodeRadius)
		{
		var nodeDeformForce = (deformNodeRadius - dist) / deformNodeRadius;
		...
		angles += nodeDeformForce * Random.onUnitSphere * maxNodeRotationStep;
