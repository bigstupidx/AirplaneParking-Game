...
// Cache for the CarWheel objects (enhancement by Brandon Lewis)

private var m_WheelFL : CarWheel;
private var m_WheelFR : CarWheel;
private var m_WheelRL : CarWheel;
private var m_WheelRR : CarWheel;


function OnEnable ()
	{
	m_Car = GetComponent(CarControl) as CarControl;
	m_WheelFL = m_Car.WheelFL;
	m_WheelFR = m_Car.WheelFR;
	m_WheelRL = m_Car.WheelRL;
	m_WheelRR = m_Car.WheelRR;
	...
	m_wheelData[0].colliderOffset = transform.InverseTransformDirection(PivotFL.position - m_WheelFL.transform.position).x;
	m_wheelData[1].colliderOffset = transform.InverseTransformDirection(PivotFR.position - m_WheelFR.transform.position).x;
	m_wheelData[2].colliderOffset = transform.InverseTransformDirection(PivotRL.position - m_WheelRL.transform.position).x;
	m_wheelData[3].colliderOffset = transform.InverseTransformDirection(PivotRR.position - m_WheelRR.transform.position).x;
...
function Update () {
	...
	DoWheelVisuals(m_WheelFL, MeshFL, PivotFL, m_wheelData[0]);
	DoWheelVisuals(m_WheelFR, MeshFR, PivotFR, m_wheelData[1]);
	DoWheelVisuals(m_WheelRL, MeshRL, PivotRL, m_wheelData[2]);
	DoWheelVisuals(m_WheelRR, MeshRR, PivotRR, m_wheelData[3]);
	...
	DoWheelPosition(m_WheelFL, PivotFL, steerL, m_wheelData[0]);
	DoWheelPosition(m_WheelFR, PivotFR, steerR, m_wheelData[1]);
	DoWheelPosition(m_WheelRL, PivotRL, 0, m_wheelData[2]);
	DoWheelPosition(m_WheelRR, PivotRR, 0, m_wheelData[3]);
...
private static function IsHardSurface(col : Collider) : boolean
	{
	var isTerrain = col.GetType() == typeof(TerrainCollider);	
	
	return !(col.sharedMaterial || isTerrain) || col.attachedRigidbody != null;
	}
