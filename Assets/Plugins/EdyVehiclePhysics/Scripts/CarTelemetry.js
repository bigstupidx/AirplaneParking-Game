...
function Update ()
	...
	if (Gizmos)
		...
		Debug.DrawLine(CoM - F, CoM + F, Color.white, 0.0, false);
		Debug.DrawLine(CoM - U, CoM + U, Color.white, 0.0, false);
		Debug.DrawLine(CoM - R, CoM + R, Color.white, 0.0, false);
		}	
	}
...
function DoWheelGizmos(Wheel : CarWheel)
	...
	if (bGrounded)
		...
		Debug.DrawLine(Hit.point, Hit.point + Wheel.transform.up * (Hit.force / 10000.0), extension <= 0.0 ? Color.magenta : Color.white, 0.0, false);
		Debug.DrawLine(Hit.point, Hit.point - Wheel.transform.forward * Hit.forwardSlip, GetSlipColor(forwardSlipRatio), 0.0, false);
		Debug.DrawLine(Hit.point, Hit.point - Wheel.transform.right * Hit.sidewaysSlip, GetSlipColor(sidewaysSlipRatio), 0.0, false);
		if (Velocity) Debug.DrawLine(Hit.point, Hit.point + Target.rigidbody.GetPointVelocity(Hit.point), Color.blue, 0.0, false);
		}
	}
